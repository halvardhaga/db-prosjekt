--THIS FILE CONTAINS ALL TRIGGERS FOR THE DATABASE.
...
WHERE NOT EXISTS (
    ...
    --TODO REMOVE: Kan bare sjekke start_time siden det ikke fins noen end_time på group_lesson BRAGE: Det finnes en end time på group lesson? Har lagt til. 
);
END;
...
BEFORE INSERT ON group_lesson_registration
BEGIN
SELECT RAISE (ABORT, 'Registration blocked: Person must have a sit membership.')
WHERE (
